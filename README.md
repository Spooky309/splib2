...
This is NOT a replacement for STL, nor is it trying to be. That said, it does not use STL (except where unavoidable, for instance, `std::initializer_list`), though we do use stdlib functions for printf (and its cousins), `string.h` as well as malloc/free/realloc, unless specific overloads for those are provided (see `SPLib.hpp`).
